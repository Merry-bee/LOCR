...
class GradNormCallback(Callback):
    ...
    def gradient_norm(model):
        ...
        return total_norm
    @staticmethod
    def average_gradient2(model):
        # return: average(w1^2+w2^2+...wn^2)
        total_norm2 = 0.0
        num_p = 0
        for p in model.parameters():
            if p.grad is not None and p.requires_grad:
                num_p += p.grad.detach().data.flatten().shape[0]
                param_norm = p.grad.detach().data.norm(2)
                total_norm2 += param_norm.item() ** 2
        average_gradient2 = total_norm2/num_p
        return average_gradient2

    def on_after_backward(self, trainer, model):
        model.log("train/average_gradient2", self.average_gradient2(model))
...
def train_prompt(config):
    ...
    model_module = PromptModelPLModule(config)
    
    data_module = PromptDataPLModule(config)
    ...
    data = datasets["train"][0].__getitem__(0)
...
if __name__ == "__main__":
    ...
    config.exp_version = (
        datetime.datetime.now().strftime("%Y%m%d_%H%M%S") if not args.exp_version else args.exp_version
    )
