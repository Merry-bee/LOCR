...
def close_table(lines:str):
    if '{table}' not in lines:
        return lines
    # 第一种情况：表头/尾缺失
    tables = re.findall(r'\\begin\{tabular\}.*?end\{table\}',lines,re.S)
    for t in tables:
        idx=lines.find(t)
        if 'begin{table}' not in lines[idx-20:idx]:
            lines=lines.replace(t,'\n\\begin{table}\n'+t)
        idx=lines.find(t)
        if 'end{tabular}' not in lines[idx:idx+len(t)]:
            lines = lines.replace(t,t.replace('\\end{table}','\\end{tabular}\n\\end{table}'))
    tables = re.findall(r'\\begin\{table\}.*?end\{tabular\}',lines,re.S)
    for t in tables:
        idx=lines.find(t)
        if 'begin{tabular}' not in lines[idx:idx+len(t)]:
            lines=lines.replace(t,t.replace('\\begin{table}','\\begin{table}\n\\begin{tabular}'))
        idx=lines.find(t)
        if 'end{table}' not in lines[idx+len(t):idx+len(t)+20]:
            lines=lines.replace(t,t+'\n\\end{table}\n')
    # 第二种情况：只开不必/只闭不开/一开多闭/一闭多开：过大的表，删除
    lines = lines.replace(' \n','\n')
    lines = lines.replace('\\begin{table}\n\n','\\begin{table}\n').replace('\\begin{tabular}\n\n','\\begin{tabular}\n').replace('\\end{tabular}\n\n','\\end{tabular}\n')+'\n\n'
    tables = re.findall(r'\\begin\{table\}.*?\n\n',lines,re.S)
    for t in tables:
        if 'end{table}' not in t or t.count('end{table}')>1 or t.count('begin{table}')>1:
            lines = lines.replace(t,'')
    tables = re.findall(r'\\begin\{tabular\}.*?\n\n',lines,re.S)
    for t in tables:
        idx = lines.find(t)
        if 'begin{table}' not in lines[idx-15:idx]:
            lines = lines.replace(t,'')
    tables = re.findall(r'.{35}\\end\{table\}',lines,re.S)
    for t in tables:
        e_idx = lines.find(t)+47
        s_idx = lines[:e_idx].rfind('\n\n')
        if 'begin{table}' not in lines[s_idx:e_idx]:
            lines = lines.replace(lines[s_idx:e_idx],'')
               
    return lines

def parenthesis_match(lines):
    lst_chars = [c for c in lines]
    dct_pairs = {'(':')','[':']','{':'}',')':'(',']':'[','}':'{'}
    
    def remove_idx(idx):
        if idx>0 and lst_chars[idx-1] == '\\':
            lst_chars[idx-1]=''
        lst_chars[idx]=''
    ...
    for index, char in enumerate(lines):
        if stack and (char=='\n' or re.match(r'\(\d+\)\n',lines[index:index+5])):
            # 一个段结束了，清空括号
            while stack:
                if index-stack[-1][1]>200:
                    break
                top_char, top_index = stack.pop()
                if lines[top_index-1] == '\\':
                    lst_chars[index-1] += '\\'    
                lst_chars[index-1] += dct_pairs[top_char]
        if char in ('(','[','{'):
            stack.append((char, index))
        elif char in (')', ']', '}'):
            if len(stack) == 0:
                remove_idx(index)
                unmatched_brackets.append((char, index))    # 多余的右括号
            else:
                top_char, top_index = stack.pop()
                if top_char != dct_pairs[char]:
                    if not stack:   # 右括号多余，左括号保留在栈内
                        stack.append((top_char,top_index))
                        remove_idx(index)
                        unmatched_brackets.append((char, index))
                    else:
                        if stack[-1][0] == dct_pairs[char]:
                            remove_idx(top_index)
                            unmatched_brackets.append((top_char, top_index))    # 认为左括号多余
                            stack.pop() # 上一个match的左括号弹出
                        else:    # 认为右括号多余，已经和两个左括号都没匹配上了
                            stack.append((top_char,top_index))
                            remove_idx(index)
                            unmatched_brackets.append((char, index))   
    ...
    while stack:
        top_char, top_index = stack.pop()
        remove_idx(top_index)
        unmatched_brackets.append((top_char, top_index))    # 多余的左括号
...
def postprocess_single(generation: str, markdown_fix: bool = True) -> str:
    ...
    if re.match(r"[A-Z0-9,;:]$", generation):
        ...
        try:
            ...
            nltk.download("words")
    # table corrections
    generation = close_table(generation)
